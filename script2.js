...
const defaultState={
  ...
  customGames:{
    ...
    line:["分からない……何も思い出せない……。何で炊けた米が部屋中に塗りたくられてるんだ……！？","それはもう、ほぼ事件じゃん。","ちょっと待って、話のサブテーマが折れた。","いま誰がこの空気の責任取るの？"],
    ...
    relo:["マーボー豆腐","チャンジャ","鮭とば","たこわさ","だし巻きたまご","サブテーマ豆","焼きナス","冷やしトマト","もつ鍋","軟骨のから揚げ"]
  }
};
let state=load();
...
const gameData={
  ...
  line:{label:"セリフ",roll:()=>`🎭 セリフ：${pick(["分からない……何も思い出せない……。何で炊けた米が部屋中に塗りたくられてるんだ……！？","それはもう、ほぼ事件じゃん。","ちょっと待って、話のサブテーマが折れた。","いま誰がこの空気の責任取るの？"])}`},
  ...
  relo:{label:"酒場",roll:()=>`🍻 レロレロ酒場：${pick(["マーボー豆腐","チャンジャ","鮭とば","たこわさ","だし巻きたまご","サブテーマ豆","焼きナス","冷やしトマト","もつ鍋","軟骨のから揚げ"])} を噛まずに言える？`}
};
const trunks=[
  "最近あった地味にムカついた話",
  "昔はダサいと思ってたけど今はアリなもの",
...
branchToLeaf.onclick=()=>{state.tree.leaf=state.tree.branch;state.logs.unshift({date:new Date().toLocaleString("ja-JP"),title:"脱線候補っぱ保存",text:state.tree.branch});save();renderHome()}
