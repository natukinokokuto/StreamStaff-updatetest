...
const seed = {
  trunk: 'まだ幹なし', branch: 'まだ枝なし', leaf: 'まだ葉なし',
  rewards: [
...
function save(){ localStorage.setItem(KEY, JSON.stringify(data)); broadcastObsSync(); render(); }

const ssObsChannel = ("BroadcastChannel" in window) ? new BroadcastChannel("ss_obs_sync") : null;
function broadcastObsSync(){
  try{
    if(!ssObsChannel) return;
    ssObsChannel.postMessage({
      type:"sync",
      timestamp:Date.now(),
      state:data
    });
  }catch(e){console.warn("OBS sync failed",e);}
}
...
function makeAutoTree(){
  ...
  data.leaf = 'コメントが来たら「葉にする」で保存';
  save();
}
function nextBranch(){ data.branch = pick(branchSeeds); save(); }
function saveLeaf(){ data.leaf = data.branch || '食いついた話題'; addLog(`🌿 葉になった話題：${data.leaf}`); }
function resetTree(){ data.trunk = 'まだ幹なし'; data.branch = 'まだ枝なし'; data.leaf = 'まだ葉なし'; save(); }
