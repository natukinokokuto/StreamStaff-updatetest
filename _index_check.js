...
(function(){
  ...
  function getSlotValue(key){
    const sel = document.querySelector('[data-obs-slot="'+key+'"]');
    if(sel) return sel.value || "空白";
    const data = readObsData();
    return (data.slots && data.slots[key]) || "空白";
  }

  function labelFor(value){
    if(!value || value === "空白") return "";
    if(value === "タイトル"){
      const t = document.getElementById("obsTitleText");
      return (t && t.value) || localStorage.getItem("ss_obs_title_text") || "";
    }
    if(value === "テロップ文"){
      const t = document.getElementById("obsTickerText");
      return (t && t.value) || localStorage.getItem("ss_obs_ticker_text") || "";
    }
    if(value === "タイトル＋時計"){
      const d = new Date();
      const time = String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0");
      const t = document.getElementById("obsTitleText");
      const title = (t && t.value) || localStorage.getItem("ss_obs_title_text") || "";
    ...
    if(value === "テロップ文＋時計"){
      ...
      const time = String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0");
      const t = document.getElementById("obsTickerText");
      const ticker = (t && t.value) || localStorage.getItem("ss_obs_ticker_text") || "";
      return ticker ? ticker+"　"+time : time;
    }
    if(value === "枠のみ") return "";
    return value;
  }
  ...
  function forcePreview(){
    ...
    // If existing renderer is working and has visible children, do not fight it.
    const visibleText = (grid.textContent || "").trim();
    if(grid.children.length >= 3 && visibleText) return;
    ...
    keys.forEach(function(key){
      ...
      const cell = document.createElement("div");
      cell.className = "obs-preview-cell";
      cell.style.border = value === "空白" ? "none" : "2px solid #555";
      cell.style.background = value === "空白" ? "transparent" : "#222";
  ...
  function schedule(){
    setTimeout(forcePreview, 50);
    setTimeout(forcePreview, 250);
  }
  ...
  document.addEventListener("change", function(e){
    if(e.target && e.target.matches("[data-obs-slot], #obsTitleText, #obsTickerText, input[type='checkbox']")){
      setTimeout(forcePreview, 50);
    }
  });
  document.addEventListener("input", function(e){
    if(e.target && e.target.matches("#obsTitleText, #obsTickerText")){
      setTimeout(forcePreview, 50);
    }
  });
})();
