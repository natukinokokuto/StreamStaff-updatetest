...
(function(){
  ...
  const slots = [
    ["topLeft", "左上", topOptions, 1, 1],
    ["topCenter", "上中央", topOptions, 1, 2],
    ["topRight", "右上", topOptions, 1, 3],
    ["midLeft", "左", midOptions, 2, 1],
    ["midCenter", "中央", midOptions, 2, 2],
    ["midRight", "右", midOptions, 2, 3],
    ["bottomLeft", "左下", bottomOptions, 3, 1],
    ["bottomCenter", "下中央", bottomOptions, 3, 2],
    ["bottomRight", "右下", bottomOptions, 3, 3]
  ];
  ...
  const presets = {
    neon:   { bg:"#050510", frame:"#00d2ff", text:"#ffffff", panel:"#111827" },
    radio:  { bg:"#160f0a", frame:"#d9a441", text:"#fff2d6", panel:"#2a1c12" },
    simple: { bg:"#000000", frame:"#555555", text:"#ffffff", panel:"#222222" },
    pink:   { bg:"#160812", frame:"#ff5bc8", text:"#ffffff", panel:"#2a1024" }
  };
  ...
  function load(){
    try{
      return JSON.parse(localStorage.getItem("ss_obs_9slot_layout") || '{"slots":{},"merge":{},"titleText":"","tickerText":"","colors":{}}');
    }catch(e){
      return {slots:{},merge:{},titleText:"",tickerText:"",colors:{}};
    }
  }

  function save(){
    const data = {slots:{}, merge:{}, titleText:"", tickerText:"", colors:{}};
    slots.forEach(([key])=>{
  ...
  function buildLayout(data){
    ...
    function row(prefix, rowNum){
      ...
      if(mergeLeft && mergeRight){
        return [
          {key:leftKey, row:rowNum, col:1, rowSpan:1, colSpan:3, hide:false}
        ];
      }

      if(mergeLeft){
        return [
          {key:leftKey, row:rowNum, col:1, rowSpan:1, colSpan:2, hide:false},
          {key:rightKey, row:rowNum, col:3, rowSpan:1, colSpan:1, hide:false}
        ];
      }

      if(mergeRight){
        return [
          {key:leftKey, row:rowNum, col:1, rowSpan:1, colSpan:1, hide:false},
          {key:centerKey, row:rowNum, col:2, rowSpan:1, colSpan:2, hide:false}
        ];
      }

      return [
        {key:leftKey, row:rowNum, col:1, rowSpan:1, colSpan:1, hide:false},
        {key:centerKey, row:rowNum, col:2, rowSpan:1, colSpan:1, hide:false},
        {key:rightKey, row:rowNum, col:3, rowSpan:1, colSpan:1, hide:false}
      ];
    }
  ...
  function init(){
    ...
    document.querySelectorAll("[data-obs-preset]").forEach(btn=>{
      btn.onclick = function(){
        const p = presets[btn.dataset.obsPreset];
        if(!p) return;
        document.getElementById("obsBgColor").value = p.bg;
        document.getElementById("obsFrameColor").value = p.frame;
        document.getElementById("obsTextColor").value = p.text;
        document.getElementById("obsPanelColor").value = p.panel;
        save();
      };
    });
